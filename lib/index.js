...
export const inject = ['webServer', 'agents', 'sessions', 'workspaceRegistry']
...
function setSessionCookie(res, sid, secret, maxAgeMs, cookieName) {
  ...
  res.setHeader(
    'set-cookie',
    `${cookieName}=${signed}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${maxAgeSec}`
  )
}
...
export function apply(ctx, config) {
  const store = new UsersStore(ctx.logger)
  const sessionsStore = new SessionsStore(ctx.logger)
  ...
  const checkCredential = (username, password) => {
    ...
    const u = store.getByUsername(username)
    if (!u || !u.enabled) return undefined
    if (store.verifyPassword(u, password)) return u
    return undefined
  }
  ...
  const getAuthUser = (req) => {
    ...
    const user = store.getByUsername(session.username)
    ...
    if (subPath === '/task') {
      ...
      try {
        ...
        const offEvent = ctx.on('session/event', (session, event) => {
          if (session.header.id === sessionId && event.type === 'assistant/message') {
            for (const block of event.data.message.content) {
              if (block.type === 'text') {
                collectedChunks.push(block.text)
              }
            }
          }
        })
        ...
        try {
          agent.prompt([{ type: 'text', text: prompt }])
          await agent.whenIdle()
        } finally {
          offEvent()
        }

        const finalOutput = collectedChunks.join('\n').trim() || '(任务已执行完成，无文本输出)'
