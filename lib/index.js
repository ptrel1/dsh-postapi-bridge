...
export const inject = ['webServer', 'agents', 'sessions', 'workspaceRegistry', 'agentDefaultModel', 'agentPresets']
    ...
    if (subPath === '/task') {
      ...
      try {
        ...
        if (!handle) {
          ...
          // 挂载与 Web 网页端完全相同的 Preset 组装（D老师专家模式/全套工具支持）
          let setupFn = undefined
          let presetName = undefined
          if (ctx.agentPresets && typeof ctx.agentPresets.resolve === 'function') {
            try {
              const resolvedPreset = await ctx.agentPresets.resolve()
              if (resolvedPreset) {
                presetName = resolvedPreset.id
                setupFn = async (agentCtx) => {
                  await ctx.agentPresets.mount(agentCtx, resolvedPreset.id)
                }
              }
            } catch (pErr) {
              ctx.logger.warning('解析全局 Preset 异常: %s', pErr)
            }
          }
          ...
          handle = await ctx.agents.create({
            ...
            meta: {
              cwd: cwd,
              ...(presetName ? { agentPreset: presetName } : {}),
            },
            setup: setupFn,
          })
