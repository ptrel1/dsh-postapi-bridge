...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Cordis 客户端插件生命周期注册
		// ============================================================================
		const name = 'server-auth';
		const inject = ['slots'];

		function apply(ctx, config) {
			// 非侵入补丁：如果当前通过非回环域名（如公网/反代）访问，放行 settings.describe 镜像
			ctx.effect(() => {
				const connection = ctx.get('connection');
				const settingsScope = ctx.get('settingsScope');
				if (settingsScope && typeof settingsScope.describe === 'function') {
					const mirror = settingsScope.describe();
					if (mirror && typeof mirror.load === 'function' && mirror.getSnapshot) {
						// 将 persistence 模式强制切换为 host，并执行一次初始化 ensure
